/*
 Circuit:
 Pressure sensor attached to pins :
 SPI2_SCK: D13
 SPI2_MISO: D12
 SPI2_MOSI: D11
 SPI2_NSS: D8
...
double pmax = 15; // maximum value of pressure range [bar, psi, kPa, etc.]
...
void setup() {
  ...
  SPI.begin();
  pinMode(PB6, OUTPUT); // pin PA11 as SS
  digitalWrite(PB6, HIGH); // set SS High
}
...
void loop() {
  ...
  while (1) {
    ...
    digitalWrite(D8, LOW); // set SS Low
    SPI.transfer(cmd, 3); // send Read Command
    digitalWrite(D8, HIGH); // set SS High
    ...
    //dtostrf converts a floating point number into a string
    ...
    temperature
    */
    sprintf(printBuffer, "The pressure is %s PSI\n", pBuff);
